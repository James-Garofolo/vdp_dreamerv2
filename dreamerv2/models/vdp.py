...
class Linear_minvar(torch.nn.Module):
    def __init__(self, in_features, out_features, bias=True, input_flag=False):
        super(Linear_minvar, self).__init__()
        self.input_flag = input_flag
        self.bias = bias
        self.mu = torch.nn.Linear(in_features, out_features, bias)
        self.sigma = torch.nn.Linear(in_features, out_features, bias)
    ...
    def forward(self, mu_x, sigma_x=torch.tensor(0., requires_grad=True)):
        if self.input_flag:
            mu_y = self.mu(mu_x)
            sigma_y = mu_x ** 2 @ (softplus(self.sigma.weight).T + minvar)
            pass
        else:
            mu_y = self.mu(mu_x)
            if len(sigma_x.shape) > 2:
                sigma_y = (softplus(self.sigma.weight) + minvar @ sigma_x.mT).mT + \
                          (self.mu.weight**2 @ sigma_x.mT).mT + \
                          (mu_x ** 2 @ (softplus(self.sigma.weight) + minvar).mT)
            else:
                sigma_y = ((softplus(self.sigma.weight) + minvar) @ sigma_x.T).T + \
                        (self.mu.weight**2 @ sigma_x.T).T + \
                        (mu_x ** 2 @ (softplus(self.sigma.weight) + minvar).T)
            pass
        if self.bias:
            sigma_y += self.sigma.bias
        sigma_y *= 1e-3
    ...
    def kl_term(self):
        kl = 0.5*torch.mean(self.mu.weight.shape[1] * (softplus(self.sigma.weight) + minvar) + torch.norm(self.mu.weight)**2
                            - self.mu.weight.shape[1] - self.mu.weight.shape[1] * torch.log((softplus(self.sigma.weight) + minvar)))
        return kl
    
class Conv2d_minvar(torch.nn.Module):
    def __init__(self, in_channels, out_channels,
                 kernel_size=(5, 5), stride=1, padding=0, dilation=1,
                 groups=1, bias=True, padding_mode='zeros',
                 input_flag=False):
        super(Conv2d_minvar, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
    ...
    def forward(self, mu_x, sigma_x=torch.tensor(0., requires_grad=True)):
        if self.input_flag:
            mu_y = self.mu(mu_x)
            vec_x = self.unfold(mu_x)
            sigma_y = (softplus(self.sigma.weight) + minvar).repeat(1, vec_x.shape[1]) @ vec_x ** 2
            sigma_y = sigma_y.view(mu_y.shape[0], mu_y.shape[1], mu_y.shape[2], mu_y.shape[3])
            pass
        else:
            mu_y = self.mu(mu_x)
            vec_x = self.unfold(mu_x)
            sigma_y = ((softplus(self.sigma.weight) + minvar).repeat(1, vec_x.shape[1]) @ self.unfold(sigma_x)) + \
                      (self.mu.weight.view(self.out_channels, -1)**2 @ self.unfold(sigma_x).permute(2, 1, 0)).permute(2, 1, 0) + \
                      ((softplus(self.sigma.weight) + minvar).repeat(1, vec_x.shape[1]) @ (vec_x ** 2).permute(2, 1, 0)).permute(2, 1, 0)
            sigma_y = sigma_y.view(mu_y.shape[0], mu_y.shape[1], mu_y.shape[2], mu_y.shape[3])
        sigma_y *= 1e-3
        return mu_y, sigma_y

    def kl_term(self):
        kl = 0.5*torch.mean(self.kernel_size * (softplus(self.sigma.weight) + minvar) + torch.norm(self.mu.weight)**2
                            - self.kernel_size - self.kernel_size * torch.log((softplus(self.sigma.weight) + minvar)))
        return kl
