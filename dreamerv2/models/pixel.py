...
class ObsEncoder(nn.Module):
    def __init__(self, input_shape, embedding_size, info):
        ...
        self.convolutions = nn.Sequential(
            nn.Conv2d(input_shape[0], d, k, padding=1),
            activation(),
            nn.Conv2d(d, 2*d, k, padding=1),
            activation(),
            nn.Conv2d(2*d, 4*d, k, padding=1),
            activation(),
        )
...
class ObsDecoder(nn.Module):
    def __init__(self, output_shape, embed_size, info):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(4*d, 2*d, k, 1, padding=1),
            activation(),
            nn.ConvTranspose2d(2*d, d, k, 1, padding=1),
            activation(),
            nn.ConvTranspose2d(d, c, k, 1, padding=1),
        )
