...
from dreamerv2.models.actor_vdp import DiscreteActionModel
from dreamerv2.models.dense_vdp import DenseModel
from dreamerv2.models.rssm_vdp import RSSM
from dreamerv2.models.pixel_vdp import ObsDecoder, ObsEncoder
from dreamerv2.utils.buffer import TransitionBuffer
from dreamerv2.models.vdp import gather_kl
...
class Trainer(object):
    ...
    def actorcritc_loss(self, posterior):
        ...
        with FreezeParameters(self.world_list+self.value_list+[self.TargetValueModel]+[self.DiscountModel]):
            ...
            imag_value_dist = self.TargetValueModel(imag_modelstate_mus, imag_modelstate_sigmas)
    ...
    def representation_loss(self, obs, actions, rewards, nonterms):
        ...
        embed_mu, embed_sigma = self.ObsEncoder(obs)                                         #t to t+seq_len   
        prev_rssm_state = self.RSSM._init_rssm_state(self.batch_size)   
        prior, posterior = self.RSSM.rollout_observation(self.seq_len, embed_mu, embed_sigma,
                                                        actions, nonterms, prev_rssm_state)
        post_modelstate_mu, post_modelstate_sigma = self.RSSM.get_model_state(posterior) #t to t+seq_len   
        obs_dist = self.ObsDecoder(post_modelstate_mu[:-1], post_modelstate_sigma[:-1]) #t to t+seq_len-1  
        reward_dist = self.RewardDecoder(post_modelstate_mu[:-1], post_modelstate_sigma[:-1]) #t to t+seq_len-1  
        pcont_dist = self.DiscountModel(post_modelstate_mu[:-1], post_modelstate_sigma[:-1]) #t to t+seq_len-1   
